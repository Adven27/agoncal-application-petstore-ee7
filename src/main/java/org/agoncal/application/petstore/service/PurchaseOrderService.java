...
public class PurchaseOrderService extends AbstractService<PurchaseOrder> implements Serializable {
    ...
    public PurchaseOrder createOrder(@NotNull Customer customer, @NotNull CreditCard creditCard, final Map<Item, Integer> cartItems) {
        ...
        for (Map.Entry<Item, Integer> cartItem : cartItems.entrySet()) {
            orderLines.add(new OrderLine(cartItem.getValue(), entityManager.merge(cartItem.getKey())));
        }
